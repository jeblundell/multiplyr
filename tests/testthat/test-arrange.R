...
test_that ("arrange() sorts on a single cluster", {
    dat <- fastdf (x=1:100, y=100:1, G=rep(c(1,2),each=50), cl=1)
    dat <- dat %>% arrange (y)
...
test_that ("arrange() maintains groups when sorting a grouped fastdf", {
    dat <- fastdf (x=1:100, y=100:1, G=rep(c("A", "B", "C", "D", each=25)), cl=2)
